...
class KuhnTrainer:
  def __init__(self, train_iterations=10**4):
    ...
    """
    alpha = 1/4
    self.avg_strategy = {'J': np.array([1-alpha, alpha]),
      'Jb': np.array([1., 0.]),
      ...
      'Qp': np.array([1., 0.]),
      'Qpb': np.array([0.666666- alpha, 0.333334+alpha])}
    """
  ...
  def train(self, n, m, memory_size):
    ...
    for iteration_t in tqdm(range(int(self.train_iterations))):
      ...
      if (len(self.M_RL_player0) == 1000 or  len(self.M_RL_player1) == 1000 or
       len(self.M_SL_player0) == 1000 or  len(self.M_SL_player1) == 1000 ):
        n, m = 2,1


      for player_i in range(self.NUM_ACTIONS):
        if player_i == 0:
          self.M_SL_player0.extend(D[0])
          self.M_RL_player0.extend(D[0])

          self.calc_best_response_value(self.avg_strategy, self.best_response_strategy, 0, "", 1)
          #FSP_Kuhn_Poker_reinforcement_learning.ReinforcementLearning().RL_train(self.M_RL_player0, player_i, self.best_response_strategy)
          FSP_Kuhn_Poker_supervised_learning.SupervisedLearning().SL_train_AVG(self.M_SL_player0, player_i, self.avg_strategy)


        else:
          self.M_SL_player1.extend(D[1])
          self.M_RL_player1.extend(D[1])

          self.calc_best_response_value(self.avg_strategy, self.best_response_strategy, 1, "", 1)
          #FSP_Kuhn_Poker_reinforcement_learning.ReinforcementLearning().RL_train(self.M_RL_player1, player_i, self.best_response_strategy)
          FSP_Kuhn_Poker_supervised_learning.SupervisedLearning().SL_train_AVG(self.M_SL_player1, player_i, self.avg_strategy)
...
#train
kuhn_trainer = KuhnTrainer(train_iterations=1000)
kuhn_trainer.train(n=100, m=50, memory_size=1000)
